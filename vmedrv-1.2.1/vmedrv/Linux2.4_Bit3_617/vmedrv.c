/* vmedrv.c */
/* VME device driver for Bit3 Model 617/618/620 on Linux 2.4.x */
/* Created by Enomoto Sanshiro on 28 November 1999. */
/* Last updated by Enomoto Sanshiro on 18 June 2001 */
...
static int probe(struct dev_prop_t* dev_prop, vmedrv_word_access_t* word_access);
...
int init_module(void)
{
    ...
    if (device_id == di616) {
	bit3.is_dma_available = 0;
    }
    else {
	bit3.is_dma_available = 1;
    }
...
static int vmedrv_ioctl(struct inode* inode, struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	uncopied_length = copy_from_user(
	    &interrupt_property, (void *) arg, argument_size
	);
	...
	result = probe(dev_prop, (vmedrv_word_access_t*) arg);
        break;
...
#if (defined(SYSTEM_REDHAT) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 4, 20)))
...
#ifndef CONFIG_PCI
#error "Kernel does not support PCI BIOS."
#endif
...
static int detect_pci_device(unsigned vendor, unsigned device, struct pci_config_t* pci_config)
{
	...
	pci_config->ioports[base_index] =
	    dev->resource[base_index].start & PCI_BASE_ADDRESS_IO_MASK;
    }
...
static int probe(struct dev_prop_t* dev_prop, vmedrv_word_access_t* word_access)
{
    ...
    result = pio_read(dev_prop, (char*) &word_access->data, address, 1);
    if (result >= 0) {
	result = 0;
    }
...
static int acknowledge_dma_interrupt(unsigned dma_status)
{
    ...
    DEBUG_DMA(printk(KERN_DEBUG "dma interrupt handled...\n"));
...
static int start_dma(struct dev_prop_t* dev_prop, unsigned long pci_address, unsigned long vme_address, unsigned long size, int direction)
{
    ...
    timeout = (size/100000 + 1) * HZ;  /* 100kB/sec; slow enough */
    remaining_time = interruptible_sleep_on_timeout(&vmedrv_dmadone_wait_queue, timeout);
    restore_flags(flags);
    if (remaining_time == 0) {
